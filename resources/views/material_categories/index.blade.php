...
@section('content')
    <div class="box">     
    <h3 class="title-5 m-b-35">Material Categories Details</h3>  
...
@include('material_categories.form');
...
<script src="https://cdn.datatables.net/buttons/2.1.0/js/buttons.print.min.js"></script>
    ...
    <script type="text/javascript">
            var table = $('#task-table').DataTable({
                ...
                ajax:  "{{ url('apiMaterialCategories') }}",
                columns: [
            ...
            $('#add_btn').on('click',function(){
                ...
                $('#form-item')[0].reset();
                $('.modal-title').text('Add Material Category');
    ...
    function editForm(id) {
        ...
        save_method = 'edit';
            ...
            $('#form-item')[0].reset();
            $.ajax({
                url:"{{ url('materialCategories') }}" + '/' + id + "/edit",
                
                success: function(html) {
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Edit Material Category');
                    $('#id').val(html.data.id);
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                ...
                if (e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('materialCategories') }}";
                    else url = "{{ url('materialCategories') . '/' }}" + id;
                    $.ajax({
                        url : url,
                        type : "POST",
