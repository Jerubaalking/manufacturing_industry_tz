...
@section('content')
    <div class="box">
        <!-- /.box-header -->
                                ...
                                <!-- DATA TABLE -->
                                <h3 class="title-5 m-b-35">Transfer History</h3>
                                <div class="table-data__tool">
                                    ...
                                    <div class="table-data__tool-right">
                                        ...
                                        <a  onclick="addForm()" class="au-btn au-btn-icon au-btn--green au-btn--small">
                                            <i class="zmdi zmdi-plus"></i>Make Transfer</a>
...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function addForm() {
            ...
            $('.modal-title').text('Make Transfer');
        }
...
//         })
            $(function(){
            $('#form-transfer').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('transfer') }}";
                    else url = "{{ url('transfer') . '/' }}" + id;
                    $.ajax({
                        url : url,
                        type : "POST",
                        //hanya untuk input data tanpa dokumen
//                      data : $('#form-account').serialize(),
                        data: new FormData($("#form-transfer")[0]),
                        contentType: false,
                        processData: false,
                        success : function(data) {
